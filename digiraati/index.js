...
//Define the port
// var port = process.env.PORT || 443; production
var port = process.env.PORT || 1443; // dev
...
fs.readFile(backup_file, function (err, data) {
  ...
  eventhandler.recover_from_backup(
    councils.get_councils(),
    users.get_all_users(),
    data["events"]
  );
...
function CheckInUser(username, council_id) {
  // Checks in a given user to a given council, needed for the "what users are present right now" functionality
...
io.on("connection", function (socket) {
  ...
  socket.on("request full data", function (user_id, callback) {
    // For the administrative panel. Returns all council data, user data and conclusion data.
  ...
  socket.on("request join council", function (data, callback) {
    ...
    if (res == -1) {
      ...
      if (res) {
        ...
        logger.AppendLog(
          "e03",
          users.get_userid_by_username(data["user"]),
          new Date().getTime(),
          data["council"]
        );
        create_backup();
  ...
  socket.on("login attempt", function (name, pw, callback) {
    if (users.login_user(name, pw, ip) == false) {
      ...
      console.log(
        "Registering new user token pair: " + user_token + ", " + uid
      );
      user_tokens.push({
        token: user_token,
        name: name,
  ...
  socket.on("request avatar change", function (data, callback) {
    ...
    users
      ...
      .set_picture("/res/" + data["avatar"] + ".png");
    logger.AppendLog(
      "e82",
      data["username"],
      new Date().getTime(),
  ...
  socket.on("request council create", function (info, callback) {
    server_log(
      ip +
        ": " +
        info["creator"] +
        " attempted to create council: " +
        info["name"]
    );
    ret_val = councils.add_council(
      info["id"],
      info["name"],
      info["description"],
      info["creator"],
    ...
    // if (info["conclusion_questions"].length > 0) - changed to typeof -check instead /petril
    if (typeof info["conclusion_questions"] !== "undefined") {
      console.log("Adding conclusion questions");
      conclusioner.add_questionnaire(info["id"], info["conclusion_questions"]);
    }

    // if (info["conclusion_base"].length > 0) { - changed to typeof -check instead /petril
    if (typeof info["conclusion_base"] !== "undefined") {
    ...
    logger.AppendLog(
      "e13",
      users.get_userid_by_username(info["creator"]),
      new Date().getTime(),
      info["id"]
    );
    create_backup();
  ...
  socket.on("request message edit", function (msg, callback) {
    if (
      councils.edit_message(msg["council"], msg["msg_id"], msg["content"]) ==
      true
    ) {
      logger.AppendLog(
        "e07",
        users.get_userid_by_username(msg["user_id"]),
        new Date().getTime(),
        msg["msg_id"]
      );
      create_backup();
  ...
  socket.on("request new message", function (msg, callback) {
    ...
    if (msg["parent"] != undefined && msg["parent"] != "") {
    ...
    if (parent == "") {
      logger.AppendLog(
        "e05",
        users.get_userid_by_username(msg["sender"]),
        new Date().getTime(),
        msg["id"]
      );
    } else {
  ...
  socket.on("request add dislike", function (data, callback) {
    ...
    var uid = users.get_userid_by_username(data["liker"]);
    var dislikes = councils.add_dislike_to_message(
      data["council"],
      data["mid"],
      uid
    );
  ...
  socket.on("request add goodarg", function (data, callback) {
    ...
    var uid = users.get_userid_by_username(data["liker"]);
    var goodargs = councils.add_goodarg_to_message(
      data["council"],
      data["mid"],
      uid
    );
  ...
  socket.on("logout attempt", function (token, callback) {
    ...
    logger.AppendLog(
      "e02",
      users.get_userid_by_username(name),
      new Date().getTime()
    );
    callback("success");
    update_page();
  ...
  socket.on("request add comment", function (data, callback) {
    ...
    if (res != -1) {
      ...
      logger.AppendLog(
        "e15",
        users.get_userid_by_username(data["sender"]),
        new Date().getTime(),
        data["file"]
      );
      create_backup();
  ...
  socket.on("request delete message", function (data) {
    ...
    logger.AppendLog(
      "e08",
      users.get_userid_by_username(data["user_id"]),
      new Date().getTime(),
      data["mid"]
    );
  ...
  socket.on("request council delete", function (data, callback) {
    server_log(
      data["submitter"] + " is deleting council " + data["council_id"]
    );
  ...
  socket.on("request logged in answers", function () {
    var returnable = conclusioner.get_answers_by_user_id(
      users.get_userid_by_username(logged_in)
    );
  ...
  socket.on("request questionnaire", function (data, callback) {
    // Palauttaa halutun käyttäjän loppukyselyn vastaukset halutulle raadille. Kutsutaan luokasta lobby_conclusion.js
    var returnable = {};
    returnable["questionnaire"] = conclusioner.get_questionnaire_by_council(
      data["council_id"]
    );
    returnable["answers"] = conclusioner.get_answers_by_user_id(
      data["council_id"],
      data["user_id"]
    );
    returnable["all_answers"] = conclusioner.get_all_answers(
      data["council_id"]
    );
    callback(returnable);
  });
  ...
  socket.on("request answers by index", function (data) {
    var returnable = conclusioner.get_answers_by_index(
      data["council_id"],
      data["index"]
    );
  ...
  socket.on("check admin access", function (token, council, callback) {
    ...
    let user_status = rightsManager.check_user_rights_in_council(
      user_id,
      council
    );
    if (user_status["role"] == "admin" || "moderator") {
      // User is an administrator and has moderation rights on every council
    ...
    if (user_status["role"] == "admin") {
      ...
      for (let i = 0; i < council_data["users"].length; i++) {
        ...
        if (fetched_data != null) {
          ...
          let role = rightsManager.check_user_rights_in_council(
            fetched_data["id"],
            council
          );
      ...
      (user_status["role"] == "moderator" ||
        user_status["role"] == "supermod") &&
      user_status["councils"].includes(council)
    ) {
      // User is a moderator or supermod on the queried council
      let returnable = {};
      let council_data = councils.get_council_data(council);
  ...
  socket.on(
    "request make user mod",
    function (user_id, promoter_token, council, callback) {
      let promoter_id = token_to_user_id(promoter_token);
      let user_rights = rightsManager.check_user_rights_in_council(
        promoter_id,
        council
      );
      if (
        user_rights["role"] == "admin" ||
        user_rights["role"] == "moderator"
      ) {
        if (
          rightsManager.add_user_to_council_mods(user_id, council) ==
            "success" ||
          "already_mod"
        ) {
          console.log(
  ...
  socket.on(
    "request make user supermod",
    function (user_id, council_id, callback) {
      if (
        rightsManager.add_user_to_council_supermods(user_id, council_id) ==
          "success" ||
        "already_supermod"
      ) {
        create_backup();
  ...
  socket.on(
    "request remove user as supermod",
    function (user_id, council_id, callback) {
      if (
        rightsManager.remove_user_from_council_supermods(user_id, council_id) ==
          "success" ||
        "not_a_supermod"
      ) {
        create_backup();
  ...
  socket.on(
    ...
    function (user_id, promoter_token, council, callback) {
      ...
      if (
        ...
        if (
          rightsManager.remove_user_from_council_mods(user_id, council) ==
            "success" ||
          "not_a_mod"
        ) {
          console.log(
  ...
  socket.on(
    "request ban user",
    function (user_id, banner_token, council, callback) {
      ...
      if (
        ...
        if (
          rightsManager.ban_user_from_council(user_id, council) == "success"
        ) {
          console.log(
            "User " +
              banner_id +
  ...
  socket.on(
    ...
    function (user_id, banner_token, council, callback) {
      ...
      if (
        ...
        if (
          rightsManager.unban_user_from_council(user_id, council) ==
            "success" ||
          "not_banned"
        ) {
          console.log(
  ...
  socket.on(
    "request admin delete message",
    function (message_id, deleter_token, council, callback) {
  ...
  uploader.on("complete", (fileInfo) => {
    ...
    logger.AppendLog(
      "e14",
      users.get_userid_by_username(fileInfo["data"]["uploader"]),
      new Date().getTime(),
      fileInfo["data"]["id"]
    );
  });
