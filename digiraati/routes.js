...
var http = require("https").createServer(
  {
    /* key: fs.readFileSync('/etc/letsencrypt/live/digiraati.fi/privkey.pem'),
        cert: fs.readFileSync('/etc/letsencrypt/live/digiraati.fi/cert.pem'),
        ca: fs.readFileSync('/etc/letsencrypt/live/digiraati.fi/chain.pem')}, app);; */
...
function checkPassword(username, password) {
  for (let i = 0; i < passwordList.length; i++) {
    if (
      passwordList[i]["username"] == username &&
      passwordList[i]["password"] == password
    ) {
      return true;
    }
  }
...
app.get("/lobby/:id/admin", function (req, res) {
  res.sendFile(__dirname + "/html/lobby_admin.html");
});
...
app.get("/socket.io-file-client.js", (req, res, next) => {
  return res.sendFile(
    __dirname + "/node_modules/socket.io-file-client/socket.io-file-client.js"
  );
});
...
app.get("/glide/:id", (req, res, next) => {
  var fid = req.params.id;
  return res.sendFile(
    path.join(__dirname, "/node_modules/@glidejs/glide/dist/" + fid)
  );
});
...
app.get("/admin", function (req, res) {
  ...
  if (checkPassword(username, password) == true) {
    console.log(
      "User " + username + " logged in to admin page at " + Date.now()
    );
    ...
    console.log(
      "User " +
        username +
        " tried to log in to admin page with incorrect password at " +
        Date.now()
    );
    return reject();
  }
