...
$(function () {
  ...
  if (
    window.sessionStorage.getItem("cookies_accepted") === null ||
    window.sessionStorage.getItem("cookies_accepted") != "true"
  ) {
    modal_open = true;
...
function create_council_face(c) {
  ...
  likes_box.appendChild(council_likes);
  likes_box.appendChild(council_likes_number);
  ...
  dislikes_box.appendChild(council_dislikes);
  dislikes_box.appendChild(council_dislikes_number);
  ...
  people_box.appendChild(council_people);
  people_box.appendChild(council_people_number);

  peukkuboksi.appendChild(likes_box);
  peukkuboksi.appendChild(dislikes_box);
  peukkuboksi.appendChild(people_box);
  ...
  if (c["userlimit"] == -1) {
    ...
    council_people_number.innerText =
      c["users"].length + " / " + c["userlimit"];
  }
  if (c["startdate"] != "" && c["enddate"] != "") {
    var council_time_box = document.createElement("div");
    council_time_box.classList.add("council_time_box");
    council_time_box.innerText =
      reformatDate(c["startdate"]) + " - " + reformatDate(c["enddate"]);
