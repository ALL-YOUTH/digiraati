var fs = require('fs');
var log_filename = __dirname + '/logs/' + new Date().toISOString().slice(-24).replace(/\D/g,'').slice(0, 14); + "-data.log";
var logs_to_parse;
...
class Datalog{
    ...
    function AppendLog(eventid, userid, timestamp, info = "")
    {
        log_json = {};
        log_json["id"] = makeid;
        log_json["eventid"] = eventid;
    ...
    function ParseLogs()
    {
        if(logs_to_parse.length > 0)
        {
            ...
            var stream = fs.createWriteStream(log_filename, {flags:'a'});
